...
//Modified by Cristóbal Ramírez for the RiVEC Benchmark Suite
...
//        unsigned long int gvl = __builtin_epi_vsetvl(NUM_TRIALS, __epi_e64, __epi_m1);
        int limit = loop_bound(SPECIES_512, NUM_TRIALS);
        swaption_seed_vector = (long*)malloc(limit*sizeof(long));
        for(int j=0; j < limit; j++) {
            swaption_seed_vector[j] = swaption_seed + j + (i * limit);
        }
        BLOCK_SIZE_AUX = limit;
#else
...
//the three rows store vol data for the three factors
    ...
    for (i = 0; i < nSwaptions; i++) {
        ...
        swaptions[i].dStrike =
                0.1 + ((int) (49 * RanUnif(&seed))) * 0.1; //strikes ranging from 0.1 to 5.0 in steps of 0.1
