...
int
HJM_Correlations(FTYPE **ppdHJMCorr,//Matrix that stores correlations among factor volatilities for different maturities
                 int iN,
                 int iFactors,
                 FTYPE **ppdFactors) {
...
int Discount_Factors_Blocking_vector(FTYPE *pdDiscountFactors, 
            int iN,
            FTYPE dYears,
            FTYPE *pdRatePath,
            int BLOCKSIZE)
...
//	unsigned long int gvl = __builtin_epi_vsetvl(BLOCKSIZE, __epi_e64, __epi_m1);
  ...
  for (j=0; j<=(iN-1)*BLOCKSIZE-1; j+=BLOCKSIZE) {
      for (b=0; b < limit; b += SPECIES_512) {
          xpdRatePath = _MM_LOAD_f64(&pdRatePath[j+b]);
          xpdRatePath = _MM_MUL_f64(xpdRatePath,xDdelt);
          xpdRatePath = _MM_EXP_f64(_MM_VFSGNJN_f64(xpdRatePath));
          _MM_STORE_f64(&pdexpRes[j+b],xpdRatePath);
      }
      // TODO scalar remainer
    }

  //initializing the discount factor vector
  for (i=0; i<(iN)*BLOCKSIZE; i+=BLOCKSIZE) {
      for (b = 0; b < limit; b+=SPECIES_512) {
          _MM_STORE_f64(&pdDiscountFactors[i+b],_MM_SET_f64(1.0));
      }
      // TODO scalar remainer
  }
  ...
  for (i=1; i<=iN-1; ++i){
      for (j=0; j<=i-1; ++j) {
          for (b = 0; b < limit; b += SPECIES_512) {
              xpdRatePath = _MM_LOAD_f64(&pdDiscountFactors[i*BLOCKSIZE+b]);
              xDdelt = _MM_LOAD_f64(&pdexpRes[j*BLOCKSIZE+b]);
              xpdRatePath = _MM_MUL_f64(xpdRatePath,xDdelt);
              _MM_STORE_f64(&pdDiscountFactors[i*BLOCKSIZE+b],xpdRatePath);
          }
      }
  }
//  FENCE();
