...
int HJM_Swaption_Blocking(FTYPE *pdSwaptionPrice, //Output vector that will store simulation results in the form:
    ...
    int iFreqRatio = (int) (dPaymentInterval / ddelt +
                            0.5);    // = ratio of time gap between swap payments and HJM step-width.
    ...
    iSwapVectorLength = (int) (iN - dMaturity / ddelt +
                               0.5);  //This is the length of the HJM rate path at the time index
    ...
    for (i = iFreqRatio; i <= iSwapTimePoints; i += iFreqRatio) {
        ...
        if (i == iSwapTimePoints)
            pdSwapPayoffs[i] = exp(
                    dStrikeCont * dPaymentInterval); //at terminal time point, bond pays coupon plus par amount
    }
    ...
    for (l = 0; l <= lTrials - 1; l += BLOCKSIZE) {
        ...
        iSuccess = HJM_SimPath_Forward_Blocking(ppdHJMPath, iN, iFactors, dYears, pdForward, pdTotalDrift, ppdFactors,
                                                iRndSeed, BLOCKSIZE_AUX); /* GC: 51% of the time goes here */
        if (iSuccess != 1)
            return iSuccess;
...
#else
        iSuccess = Discount_Factors_Blocking(pdPayoffDiscountFactors, iN, dYears, pdDiscountingRatePath,
                                             BLOCKSIZE_AUX); /* 15% of the time goes here */
        ...
        //        unsigned long int gvl = __builtin_epi_vsetvl(BLOCKSIZE_AUX, __epi_e64, __epi_m1);
                int limit = loop_bound(SPECIES_512, BLOCKSIZE_AUX);
                _MMR_f64    xpdSwapDiscountFactors;
                _MMR_f64    xpdSwapPayoffs;
                _MMR_f64    xdFixedLegValue             = _MM_SET_f64(0.0);
                ...
                for (b = 0; b < limit; b+=SPECIES_512) {
                    xdFixedLegValue = _MM_SET_f64(0.0);
                    for (i=0;i<=iSwapVectorLength-1;++i){
                          xpdSwapDiscountFactors = _MM_LOAD_f64(&pdSwapDiscountFactors[i*BLOCKSIZE_AUX+b]);
                          xpdSwapPayoffs = _MM_SET_f64(pdSwapPayoffs[i]);
                          xdFixedLegValue = _MM_MACC_f64(xdFixedLegValue,xpdSwapPayoffs,xpdSwapDiscountFactors);
                      }

                    xdFixedLegValue = _MM_MAX_f64(_MM_SUB_f64(xdFixedLegValue,oNE), zero);
                    xdFixedLegValue = _MM_MUL_f64(xdFixedLegValue,_MM_SET_f64(pdPayoffDiscountFactors[iSwapStartTimeIndex*BLOCKSIZE_AUX]));
...
//                xdSumSimSwaptionPrice       = _MM_SET_f64(dSumSimSwaptionPrice);
//                xdSumSquareSimSwaptionPrice = _MM_SET_f64(dSumSquareSimSwaptionPrice);

                // accumulate into the aggregating variables =====================
                    dSumSimSwaptionPrice = _MM_REDSUM_f64(xdFixedLegValue);
                    dSumSquareSimSwaptionPrice = _MM_REDSUM_f64(_MM_MUL_f64(xdFixedLegValue,xdFixedLegValue));
...
//                FENCE();
        ...
        for (b = 0; b < BLOCKSIZE_AUX; b++) {
            ...
            dSwaptionPayoff = dFixedLegValue - 1.0 > 0 ? dFixedLegValue - 1.0 : 0; // dMax(dFixedLegValue - 1.0, 0);
