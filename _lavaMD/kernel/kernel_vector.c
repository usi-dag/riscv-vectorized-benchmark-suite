...
//#pragma omp	parallel for \
    ...
    for (l = 0; l < dim.number_boxes; l = l + 1) {
        ...
        for (k = 0; k < (1 + box[l].nn); k++) {
            ...
            fp *rBv = (fp *) malloc(NUMBER_PAR_PER_BOX * sizeof(fp));
            fp *rBx = (fp *) malloc(NUMBER_PAR_PER_BOX * sizeof(fp));
            fp *rBy = (fp *) malloc(NUMBER_PAR_PER_BOX * sizeof(fp));
            fp *rBz = (fp *) malloc(NUMBER_PAR_PER_BOX * sizeof(fp));
...
//				unsigned long int gvl = __builtin_epi_vsetvl(NUMBER_PAR_PER_BOX, __epi_e32, __epi_m1);
                ...
                int limit = loop_bound(INT32_SPECIES_512, NUMBER_PAR_PER_BOX);
                _MMR_f32 xr2;
                _MMR_f32 xDOT;
                _MMR_f32 xu2;
                _MMR_f32 xa2 = _MM_SET_f32(a2);
                _MMR_f32 xvij;
                _MMR_f32 xrA_v = _MM_SET_f32(rA[i].v);
                _MMR_f32 xrA_x = _MM_SET_f32(rA[i].x);
                _MMR_f32 xrA_y = _MM_SET_f32(rA[i].y);
                _MMR_f32 xrA_z = _MM_SET_f32(rA[i].z);
                _MMR_f32 xrB_v;
                _MMR_f32 xrB_x;
                _MMR_f32 xrB_y;
                _MMR_f32 xrB_z;
                ...
                _MMR_f32 xfs;
                _MMR_f32 xqB;
                _MMR_f32 xfA_v = _MM_SET_f32(0.0);
                _MMR_f32 xfA_x = _MM_SET_f32(0.0);
                _MMR_f32 xfA_y = _MM_SET_f32(0.0);
                _MMR_f32 xfA_z = _MM_SET_f32(0.0);
                fp xfA_1_v = 0.0; //_MMR_f32 xfA_1_v = _MM_SET_f32(0.0,1);
                fp xfA_1_x = 0.0; //_MMR_f32 xfA_1_x = _MM_SET_f32(0.0,1);
                fp xfA_1_y = 0.0; //_MMR_f32 xfA_1_y = _MM_SET_f32(0.0,1);
                fp xfA_1_z = 0.0; //_MMR_f32 xfA_1_z = _MM_SET_f32(0.0,1);

                // do for the # of particles in current (home or neighbor) box
                for (j = 0; j < limit; j += INT32_SPECIES_512) {
//					gvl = __builtin_epi_vsetvl(NUMBER_PAR_PER_BOX-j, __epi_e32, __epi_m1);
                    // coefficients
                    xrB_v = _MM_LOAD_f32(&rBv[j]); // _MM_LOAD_STRIDE_f32(&rB[j].v,4,gvl);
                    xrB_x = _MM_LOAD_f32(&rBx[j]); // _MM_LOAD_STRIDE_f32(&rB[j].x,4,gvl);
                    xrB_y = _MM_LOAD_f32(&rBy[j]); // _MM_LOAD_STRIDE_f32(&rB[j].y,4,gvl);
                    xrB_z = _MM_LOAD_f32(&rBz[j]); // _MM_LOAD_STRIDE_f32(&rB[j].z,4,gvl);
                    ...
                    //vij= exp(-u2);
                    xvij = _MM_EXP_f32(_MM_VFSGNJN_f32(xu2));
                    //fs = 2.*vij;
                    xfs = _MM_MUL_f32(_MM_SET_f32(2.0f), xvij);
                    //d.x = rA[i].x  - rB[j].x;
                    xd_x = _MM_SUB_f32(xrA_x, xrB_x);
                    //d.y = rA[i].y  - rB[j].y;
                    xd_y = _MM_SUB_f32(xrA_y, xrB_y);
                    //d.z = rA[i].z  - rB[j].z;
                    xd_z = _MM_SUB_f32(xrA_z, xrB_z);
                    //fxij=fs*d.x;
                    xfxij = _MM_MUL_f32(xfs, xd_x);
                    //fyij=fs*d.y;
                    xfyij = _MM_MUL_f32(xfs, xd_y);
                    //fzij=fs*d.z;
                    xfzij = _MM_MUL_f32(xfs, xd_z);
...
//				gvl = __builtin_epi_vsetvl(NUMBER_PAR_PER_BOX, __epi_e32, __epi_m1);
                ...
                xfA_1_v += _MM_REDSUM_f32(xfA_v);
                xfA_1_x += _MM_REDSUM_f32(xfA_x);
                xfA_1_y += _MM_REDSUM_f32(xfA_y);
                xfA_1_z += _MM_REDSUM_f32(xfA_z);
                fA[i].v += xfA_1_v; // _MM_STORE_f32(&fA[i].v, xfA_1_v,1);
                fA[i].x += xfA_1_x; // _MM_STORE_f32(&fA[i].x, xfA_1_x,1);
                fA[i].y += xfA_1_y; // _MM_STORE_f32(&fA[i].y, xfA_1_y,1);
                fA[i].z += xfA_1_z; // _MM_STORE_f32(&fA[i].z, xfA_1_z,1);
